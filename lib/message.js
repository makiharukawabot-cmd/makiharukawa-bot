imimport { proto, delay, areJidsSameUser, generateWAMessage, prepareWAMessageMedia, generateWAMessageFromContent, downloadContentFromMessage, generateMessageID, generateWAMessageContent, getContentType, getDevice, extractMessageContent, } from '@whiskeysockets/baileys';
...
const canalSeleccionado = canales[Math.floor(Math.random() * canales.length)]
  ...
  const namebugsito = botSetting.botname || ''
  const namebugg2 = botSetting.namebot || ''
  const pfp = botSetting.icon || ''
  const canalI = botSetting.id || ''
  ...
  client.sendContextInfoIndex = async (
    ...
    const canalId = settings.id || ''
    const canalName = settings.nameid || ''
    const sourceUrl = typeof config.redes === 'string' ? config.redes : typeof settings.link === 'string' ? settings.link : 'https://github.com/The-King-Destroy'
  ...
  return m
}port { proto, delay, areJidsSameUser, generateWAMessage, prepareWAMessageMedia, generateWAMessageFromContent, downloadContentFromMessage, generateMessageID, generateWAMessageContent, getContentType, getDevice, extractMessageContent, } from '@whiskeysockets/baileys';
import { resolveLidToRealJid } from "./utils.js"
import chalk from 'chalk';
import fs from 'fs';
import axios from 'axios';
